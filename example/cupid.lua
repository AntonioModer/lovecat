...
local config = {
	...
	watcher_onchanged = "reload(); lovecat.reload()",
...
local function retaining(...)
	...
	for k,v in pairs(values) do g["set" .. k](unpack(v)) end
	g.pop()
end

local function clear_screen()
	retaining('Scissor', function()
		g.setScissor()
		g.clear()
	end)
end
...
cupid_commands = {
	...
	["command"] = function(self, cmd)
		local xcmd = cmd
		if not (
			xcmd:match("end") or xcmd:match("do") or xcmd:match("function")
			or xcmd:match("return") or xcmd:match("=") or
			xcmd:match(';')
		) then
			xcmd = "return " .. xcmd
		end
...
local function love_graphics_pop_all()
	while true do
		love.graphics.pop()
	end
end
...
mods.console = function() return {
	...
	["post-draw"] = function(self)
		...
		retaining("Color","Font","Canvas","BlendMode","Shader","LineWidth", function()
			g.origin()
			g.setCanvas()
			g.setShader()
...
mods.error = function() return {
	...
	["post-draw"] = function(self)
		...
		retaining("Color", "Font", "BlendMode", "Shader", "Canvas", "LineWidth", function()
			g.origin()
			g.setBlendMode('alpha')
			g.setShader()
			g.setLineWidth(1)
